...
describe('xAI API key provider', () => {
  test('normalizes the backend xai-api-key contract and exposes a workbench resource', () => {
    ...
    expect(resource.models).toEqual(['grok-latest', 'grok-4.5']);
    expect(resource.modelDisplays).toEqual(['grok-latest']);
    expect(resource.modelSearchTerms).toEqual(['grok-latest', 'grok-4.5']);
